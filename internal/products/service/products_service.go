...
func (s *Service) CreateProduct(ctx context.Context, userId, companyId uuid.UUID, req domain.CreateProductRequest) (domain.ProductResponse, error) {
	...
	product, err := s.repo.CreateProduct(ctx, db.CreateProductParams{
		...
		Ncm:                   pgconv.ParseStringToPgText(req.Ncm),
		Cest:                  pgconv.ParseStringToPgText(req.Cest),
		Csosn:                 pgconv.ParseStringToPgText(req.Csosn),
		CfopSaidaDentroEstado: pgconv.ParseStringToPgText(req.CfopSaidaDentroEstado),
		CfopSaidaForaEstado:   pgconv.ParseStringToPgText(req.CfopSaidaForaEstado),
		OrigemMercadoria:      req.OrigemMercadoria,
	})
	if err != nil {
...
func (s *Service) GetProductById(ctx context.Context, id uuid.UUID) (domain.ProductResponse, error) {
	...
	return domain.ProductResponse{
		...
		UpdatedAt:             pgconv.PgTimestamptzToTime(product.UpdatedAt),
		DeletedAt:             pgconv.PgTimestamptzToTime(product.DeletedAt),
		Ncm:                   pgconv.ParsePgTextToString(product.Ncm),
		Cest:                  pgconv.ParsePgTextToString(product.Cest),
		Csosn:                 pgconv.ParsePgTextToString(product.Csosn),
		CfopSaidaDentroEstado: pgconv.ParsePgTextToString(product.CfopSaidaDentroEstado),
		CfopSaidaForaEstado:   pgconv.ParsePgTextToString(product.CfopSaidaForaEstado),
		OrigemMercadoria:      product.OrigemMercadoria,
	}, nil
}
...
func (s *Service) UpdateProduct(ctx context.Context, id uuid.UUID, req domain.UpdateProductRequest) (domain.ProductResponse, error) {
	...
	arg := db.UpdateProductParams{
		...
		Csosn:                 currentProduct.Csosn,
		CfopSaidaDentroEstado: currentProduct.CfopSaidaDentroEstado,
		CfopSaidaForaEstado:   currentProduct.CfopSaidaForaEstado,
		OrigemMercadoria:      currentProduct.OrigemMercadoria,
	}
